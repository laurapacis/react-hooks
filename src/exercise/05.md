...
DOM nodes aren’t created at all until the ReactDOM.render method is called. Your
...
But often when working with React we need to integrate with UI libraries. Some
of these need to work directly with the DOM. So to get access to the DOM, we
need to ask React to give us access to a particular DOM node when it renders our
component. The way this happens is through a special prop called ref.

**The useRef is a hook that allows to directly create a reference to the DOM
element in the functional component.**

In review, anytime we need to interact with the DOM, that interaction is a side
effect. We're going to be using useEffect for that. To get access to a DOM node,
we use React.useRef and this special ref prop to get access to the DOM node that
React creates for this element.

**Basically, we use the ref hook. Anytime we want to maintain a reference to
something, and we want to be able to make changes to that thing without
triggering a rerender.**  If we want to trigger a rerender, then we should
probably just use state instead.
