...
**Lifting state** means finding the closest common parent shared between two
components, placing the state management there, and passing the state and a
mechanism for updating that state down into the components that need it. ––
Easier to visualize with the React potato tree mental model.

**Colocating state** ––the opposite–– means pushing state back down, without
forgetting to clean up the components that don't need that state anymore. It is
important to keep the state as close as possible to where it's relevant, since
this improves performace for the application. Note: The updated component no
longer accepts props, and it can encapsulate the management of its own state by
itself.
