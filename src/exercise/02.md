...
This hook handles "side-effects" i.e. anything that causes changes _outside_ of
rendering your function to the DOM, e.g.: timers, fetching data, changing scroll
position, etc. In this practice specially Local Storage.

useEffect is a built-in hook that accepts a callback function as argument that
React will call after the DOM is updated.

### 1. Lazy state initialization

For better performance, React allows to pass a function instead of a value in
useState, so React will only call that function to get the state value when the
component is rendered for the first time.

### 2. Effect dependencies

Array passed at the second argument of the useEffect function, which signals to
React that your effect callback function should be called _only_ when those
dependencies change. It avoids doing unnecessary work.

### 3. Custom hooks

Same as in JavaScript, with React you can isolate a bit of logic in custom
function/hook and reuse it somewhere else in your app.
