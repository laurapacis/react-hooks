...
We add a status element of state, which we keep updated as our promise
progresses. Then we render different UI based on that status variable, helping
us avoid bugs, and making the code a lot more clear.

**Store the State in an Object**

That helps us avoid issues were calling a state updater function in an
asynchronous handler like this is going to trigger multiple re-renders. We may
not have set all of the state that we needed to when that happens. However, it's
not always recommended putting the state into a single object like this, because
that's not often a problem. –– But it is good to understand that this is a
potential issue; a better alternative it is the hook `useReducer`.
