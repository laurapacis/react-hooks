...
React Hooks are special functions that hold state in interactive applications.

React has a series of built-in hooks (useState, useEffect, useRef, useContext,
useReducer – each of them has a unique API) but you can also create your own
custom hooks.

Each hook is a function that you can call inside your own custom React component
function to store data (=state) or to perform actions (=side effects).

---

React.useState is a function that accepts an only argument, which is the initial
state for the instance of the component. React.useState returns a pair of values
= arrays of two elements --> we use destructuring syntax to assign those two
values into two different variables.

The first value is the state value, and the second is a function we call to
update the state.

Common naming is for instance: fish, setFish.

State can be define as: data that changes over time i.e. for instance, when we
have a state value (fish), and we call the function to update this state
(setFish), the component function is re-run, so the state we get this time is
the value we got back when we called setFish, which will be updated to another
value to another value to another value everytime we call the setFish funciont
to update state. This will end when we demount the component –or the user closes
the app.
